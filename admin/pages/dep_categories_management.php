...
if (isset($_SESSION['id']) and $_SESSION['status'] == 1) {
    ...
    if ($section == "home") {
        ...
        $sql = "SELECT * FROM dep_categories";
        $result = mysqli_query($conn, $sql);
        ?>

        <div class="p-4">
            <div>
                <a class="btn btn-success" href="?section=add&id=<?php echo $id ?>">اضافة فئة جديدة</a>
            </div>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        <div
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <p>الراتب: <span><?php echo $row['salary'] ?></span></p>
                            </div>
                            <div>
        ...
        if (isset($_POST['send'])) {
            ...
            if (!empty($name) && !empty($salary)) {
                ...
                $sql = "INSERT INTO dep_categories(name, salary, dep_id) VALUES ('$name', '$salary', '$id')";
                ...
                if (mysqli_query($conn, $sql)) {
                    ...
                    <?php
                    header("refresh:2;url=dep_categories_management.php?id=" . $id);
                } else {
        ...
        <div class="my-container">
            <div class="d-flex w-100 mt-4 justify-content-between align-items-center">
                <h4 class="title-text">اضافة فئة جديدة</h4>
            </div>
            <form action="?section=add&id=<?php echo $id ?>" method="post" enctype="multipart/form-data"
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="number" name="salary" id="salary" class=" input" id="exampleFormControlTextarea1" rows="3"
                        required />
                </div>
        ...
        require '../../connect.php';
        $sql = "SELECT * FROM dep_categories WHERE id = '$id'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        mysqli_close($conn);
        ...
        $sql = "DELETE FROM dep_categories WHERE id = '$id'";
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            header("refresh:2;url=dep_categories_management.php");
        } else {
